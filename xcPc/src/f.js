...
//日期传2020-01-04 这种格式
Vue.prototype.dateTime = function (v) {
    console.log(v)
    let date = new Date(v)
    return date.getTime();
}
...
//比较当前日期是否在两个日期中间
Vue.prototype.compareDate = function (sTime, eTime) {
    console.log(eTime)
    if ((Vue.prototype.dateTime(sTime) < new Date().getTime()) && (Vue.prototype.dateTime(eTime) > new Date().getTime())) {
        return false
    }
    // console.log(Vue.prototype.dateTime(sTime))
    // console.log(Vue.prototype.dateTime(eTime))
...
//手机号隐藏中间五位数
Vue.prototype.HiddenRule = function (phone) {
    return phone.substr(-11, 3) + phone.substr(3, phone.length - 8).replace(/\w/g, '*') + phone.substr(phone.length - 4)
}
