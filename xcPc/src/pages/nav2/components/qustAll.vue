<template>
  <Row :gutter="16">
    <!-- <i-col :span="24"> -->
    <i-col :span="23" style="margin-top:5px">
      <Form :model="formItem" :label-width="80">
        ...
        <i-col span="4" offset="" style="margin-left: -50px">
          <FormItem>
            <Button
              type="primary"
              @click="search"
        ...
        <i-col span="2" offset="" style="float:right">
          <Button
            type="primary"
            style="background: #219480;color: #fff;"
            class="search"
    ...
    >
      <i-col span="18">
        <Form :model="formItem" :label-width="80">
          <FormItem label="问题" style="margin-top: 25px">
            <Input
...
export default {
  ...
  data() {
    return {
      questionTitle:'' ,             //新增、编辑问题弹框标题
      hoodleMobileList:[], //问题内容 对应 = >所有选择的问题列表
      total: 0,
      page: 1,
      page_Size: 10,
      data1: [],
                ...
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      getHoodleMobileList(data3).then(res => {   //重新获取所有多轮会话列表    =>文本列表
                        console.log(res);
                        if (res.code == "10000") {
                      ...
                      if (row.ztKnowAnswers[0].type == "1") {
                        ...
                        this.formItem.hoodleMobileids =
                         ...
                         // this.hoodleMobileList.forEach((v,i)=>{            //问题内容回旋
                         //   if(v.hoodleMobileid==row.ztKnowAnswers[0].content){
                         //     this.formItem.hoodleMobileids = v.hoodleMobileadress
                         //     console.log("测试啦")
                         //     console.log(row.ztKnowAnswers[0].content)
                         //     console.log(v.hoodleMobileid)
                         //     console.log(this.formItem.hoodleMobileids)
                         //   }
                         // })
                      ...
                      this.formItem.isbussiness = String(row.isbussiness); // 业务问题
  ...
  methods: {
    ...
    showDrawer() {
      ...
      getHoodleMobileList(data3).then(res => {        //获取所有多轮会话列表    =>文本列表
        console.log(res);
        if (res.code == "10000") {
          this.showcontentList = res.content;
        }
    ...
    saveBtn() {
      ...
      } else if (this.formItem.jumpType == "9") {
      ...
      let data = {
        ...
        keywords: this.formItem.keywords =  this.formItem.keywords?(this.keywordsList?(this.formItem.keywords +
         "," ):this.formItem.keywords):''
            +
      ...
      } else if (!data.ztKnowType.typeid && data.ztKnowAnswers[0].type != "9") {
        this.$Message.error("请选择类型");
      } else if (
        this.formItem.answerType == "2" &&
        !data.ztKnowAnswers[0].remark &&
        data.ztKnowAnswers[0].type != "9"
      ) {
